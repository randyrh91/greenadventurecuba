 ...
 * User: pr0x
 */
$model = new ImagenAlojamientoViewModel();
$item = $model->getImagenAlojamiento();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando ImagenAlojamiento: <?= __($item->__toString()) ?></h2>
    <?php else: ?>
        <h2>Agregando ImagenAlojamiento</h2>
    <?php endif ?>
    <form method="post">
        <input type="hidden" name="controller" value="ImagenAlojamiento.save">
        ...
        <table class="form-table">
            <tbody>
				...
				<tr class="wrap">
					...
					<td>
						<?=HTML::dropDownList('Alojamiento_id',$model->getAlojamientos(),$item->id)?>
					</td>
				</tr>
		</tbody>
        </table>
