...
$item = $model->getTipoAlojamiento();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando TipoAlojamiento: <?= __($item->__toString()) ?></h2>
    <?php else: ?>
        <h2>Agregando TipoAlojamiento</h2>
    <?php endif ?>
    <form method="post">
        <input type="hidden" name="controller" value="TipoAlojamiento.save">
        ...
        <div class="right">
            ...
            <?= HTML::link_buttonVM('Ver Listado', 'cs_mvc_tipo_alojamientos') ?>
        </div>
    </form>
</div>
<script>
