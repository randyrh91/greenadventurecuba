...
$item = $model->getImagenExcursion();
?>
<div class="wrap">
    <?php if ($item->id): ?>
        <h2>Editando ImagenExcursion: <?= __($item->__toString()) ?></h2>
    <?php else: ?>
        <h2>Agregando ImagenExcursion</h2>
    <?php endif ?>
    <form enctype="multipart/form-data" method="post">
        <input type="hidden" name="controller" value="ImagenExcursion.save">
        ...
        <div class="right">
            ...
            <?= HTML::link_buttonVM('Ver Listado', 'cs_mvc_imagen_excursiones') ?>
        </div>
    </form>
</div>
<script>
